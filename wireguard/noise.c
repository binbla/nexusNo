...
static struct noise_keypair* keypair_create(struct wg_peer* peer) {
    ...
    if (unlikely(!keypair)) return NULL;
...
void wg_noise_expire_current_peer_keypairs(struct wg_peer* peer) {
    ...
    if (keypair) keypair->sending.is_valid = false;
    keypair = rcu_dereference_protected(
...
bool wg_noise_received_with_keypair(struct noise_keypairs* keypairs,
    ...
    if (likely(!key_is_new)) return false;
...
static void hmac(u8* out, const u8* in, const u8* key, const size_t inlen,
    ...
    } else memcpy(x_key, key, keylen);

    for (i = 0; i < BLAKE2S_BLOCK_SIZE; ++i) x_key[i] ^= 0x36;
    ...
    for (i = 0; i < BLAKE2S_BLOCK_SIZE; ++i) x_key[i] ^= 0x5c ^ 0x36;
...
static void kdf(u8* first_dst, u8* second_dst, u8* third_dst, const u8* data,
    ...
    if (!first_dst || !first_len) goto out;
    ...
    if (!second_dst || !second_len) goto out;
    ...
    if (!third_dst || !third_len) goto out;
...
static bool __must_check mix_dh(u8 chaining_key[NOISE_HASH_LEN],
    ...
    if (unlikely(!curve25519(dh_calculation, private, public))) return false;
...
static bool __must_check mix_precomputed_dh(
    u8 chaining_key[NOISE_HASH_LEN], u8 key[NOISE_SYMMETRIC_KEY_LEN],
...
static void message_encrypt(u8* dst_ciphertext, const u8* src_plaintext,
    ...
    chacha20poly1305_encrypt(dst_ciphertext, src_plaintext, src_len, hash,
                             NOISE_HASH_LEN, 0 /* Always zero for Noise_IK */,
                             key);
...
bool wg_noise_handshake_create_initiation(
    ...
    if (unlikely(!handshake->static_identity->has_identity)) goto out;
...
struct wg_peer* wg_noise_handshake_consume_initiation(
    struct message_handshake_initiation* src, struct wg_device* wg) {
    ...
    if (unlikely(!wg->static_identity.has_identity)) goto out;
    ...
    if (replay_attack || flood_attack) goto out;
...
bool wg_noise_handshake_create_response(struct message_handshake_response* dst,
    ...
    if (handshake->state != HANDSHAKE_CONSUMED_INITIATION) goto out;
...
struct wg_peer* wg_noise_handshake_consume_response(
    struct message_handshake_response* src, struct wg_device* wg) {
    ...
    handshake = (struct noise_handshake*)wg_index_hashtable_lookup(
        wg->index_hashtable, INDEX_HASHTABLE_HANDSHAKE, src->receiver_index,
        &peer);
    if (unlikely(!handshake)) goto out;
    ...
    if (state != HANDSHAKE_CREATED_INITIATION) goto fail;

    /* e */
    message_ephemeral(e, src->unencrypted_ephemeral, chaining_key, hash);

    /* ee */
    if (!mix_dh(chaining_key, NULL, ephemeral_private, e)) goto fail;
...
bool wg_noise_handshake_begin_session(struct noise_handshake* handshake,
    ...
    rcu_read_lock_bh();
    if (likely(!READ_ONCE(
            container_of(handshake, struct wg_peer, handshake)->is_dead))) {
