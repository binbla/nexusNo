...
// H_i = HASH(H_i || rs) 这里的rs就是remote_static
// 实际上就是第三个公式
// 在生成ephemeral 公钥私钥之前。
void NoiseProtocol::handshake_init(Handshake& hs) {
    ...
    mix_hash(hs.hash, hs.remote_static);
}
...
inline void NoiseProtocol::derive_keys(DirectionalKey& first_dst,
                                       ...
                                       uint64_t birthdate) {
    crypto_.kdf2(chaining_key, std::span<const uint8_t>{}, first_dst.key,
                 second_dst.key);
...
bool NoiseProtocol::create_initiation(Peer& peer, KeypairIndex local_index,
    ...
    */
    ...
    Hash hash{};
    SymmetricKey ep_key{};
    handshake_init(chaining_key, hash, peer.remote_static());
    ...
    crypto_.generate_ephemeral_keypair(ep_key, out.ephemeral_public);

    // 4. e
    // ck = Kdf1(ck, Ei_pub)
    // h = Hash(h || Ei_pub)
    message_ephemeral(out.ephemeral_public, out.ephemeral_public, chaining_key,
                      hash);

    // 5. es
    // (ck, key) = mix_dh(ck, e_priv, rs)
    SymmetricKey key{};
    if (!mix_dh(chaining_key, key, ep_key, peer.remote_static())) {
        return false;
    }

    // 6. s
    // msg.static_encrypted = AEAD(key, h, local_public)
    message_encrypt(out.static_encrypted, local_public_, key, hash);
    ...
    message_encrypt(out.timestamp_encrypted, timestamp, key, hash);

    // 提交
    // 9. 记录本端索引
    Handshake& hs = peer.handshake();  // 返回的是引用。
    hs.hash = hash;
...
Peer* NoiseProtocol::consume_initiation(const HandshakeInitiation& msg,
    ...
    // 写一个临时的握手状态，但是不使用hs结构体了。
    Hash hash = handshake_init_hash;                         // H_i
    ChainingKey chaining_key = handshake_init_chaining_key;  // C_i
    mix_hash(hash, local_public_);                           // 混入我的公钥

    // 2. e 对方的临时公钥，这是明文
    e = msg.ephemeral_public;
    message_ephemeral(e, e, chaining_key, hash);
    // mix e 到 hash 和 chaining_key

    // 3. es = DH(Sr_priv, Ei_pub)
    // 取得了对方ephemeral公钥，同时我们有自己的私钥，就可以计算出
    // DH 结果了，这个结果是后续解密和认证的基础
    if (!mix_dh(chaining_key, key, local_private_, e)) {
        return nullptr;
    }
    ...
    // 找到了就先拿到peer的握手状态，准备更新
    Handshake& hs = peer->handshake();  // 从peer拿到的引用，只需要更新c和h

    // 6. ss = precomputed DH(Sr_priv, Si_pub)
    if (!mix_precomputed_dh(chaining_key, key,
                            peer->precomputed_static_static())) {
        return nullptr;
    }
